...
class Conversation:
    ...
    def getOllamaFormattedMessages(self):
        
        if self.rag_prompt is None:
            
            message = self.messages[0][1]
            if type(message) is tuple:
            ...
            if i % 2 == 0:
                ...
                if type(message) is tuple:
                    ...
                    formattedMessages.append(
                        {
                            'role': 'user',
                            'content': text if i != 0 else self.rag_prompt,
                            'images': [image_base64],
                        }
                    )
                else:
                    formattedMessages.append({
                            'role': 'user',
                            'content': message if i != 0 else self.rag_prompt
                        }) 
            else:
                formattedMessages.append({
                            'role': 'assistant',
...
default_conversation = Conversation(
    system="Чат между любопытным пользователем и помощником-искусственным интеллектом. "
    "Помощник дает полезные, подробные и вежливые ответы на вопросы пользователя."
    "Отвечай только на русском языке. Если будешь отвечать на другом языке, твой ответ будет неверным.",
    roles=("USER", "ASSISTANT"),
    version="v1",
    messages=(),
    offset=0,
